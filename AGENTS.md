...
```json
{
  ...
  "settings": {
    ...
    "autoOutboundsInterface": "<physical interface name or auto>"
  },
  "sniffing": {
    "enabled": true,
...
DNS for packets from `tun-in` is routed before the generic TUN rule: TCP/UDP port 53 goes to `dns-out`, and the runtime config sets Xray DNS servers to `1.1.1.1` and `8.8.8.8`. Do not send TUN DNS directly to `vless-reality`; Windows can otherwise keep using link-local/router DNS addresses and name resolution will fail or leak outside the intended path.

PowerShell helper output is forced to UTF-8 before JSON parsing. This matters for localized Windows interface aliases: corrupt adapter names passed into `autoOutboundsInterface` break Xray outbound binding and make normal traffic stay on the physical default route.

Маршрутизация вставляет сначала правило `tun-in` port `53` -> `dns-out`, затем общее правило `tun-in -> vless-reality`, чтобы TUN-трафик не перехватывался более ранними direct/block правилами базового профиля. `autoOutboundsInterface` получает найденное имя физического интерфейса или `"auto"` как fallback, чтобы снизить риск петли, когда трафик самого Xray попадает обратно в TUN.

Перед запуском TUN приложение определяет физический маршрут до proxy endpoint, добавляет временный host-route только до IP сервера (`/32` или `/128`) через физический gateway/interface и передает найденный интерфейс в `autoOutboundsInterface`. Это предотвращает routing loop, когда Xray пытается подключиться к своему серверу через собственный TUN. Full-tunnel маршруты не добавляются Python/PowerShell-кодом; ими управляет Xray через `autoSystemRoutingTable`.
