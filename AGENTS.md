...
Для Windows full-tunnel используется split-default вместо обычного `0.0.0.0/0`: маршруты `0.0.0.0/1` и `128.0.0.0/1` специфичнее стандартного default route и надежнее выигрывают в таблице маршрутизации. После запуска Xray приложение дополнительно проверяет/добавляет активные маршруты через `xstart0` с помощью PowerShell `New-NetRoute`, с fallback на `netsh`.
