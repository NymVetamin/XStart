...
`.gitignore` уже игнорирует `venv`, `output`, `geoip.dat`, `geosite.dat`, `xray.exe`, `test.py`.
...
Для полноценной проверки запуска Xray рядом с `main.py` должен быть доступен `xray.exe`, либо `xray.exe` должен находиться в `PATH`. Для правил маршрутизации в генерируемом конфиге нужны `geoip.dat` и `geosite.dat` из релиза Xray-core.
...
После сборки проверить, что рядом с итоговым `XStart.exe` лежат внешние файлы Xray (`xray.exe`, `geoip.dat`, `geosite.dat`). Они не встраиваются текущей логикой.
...
- Не коммитьте runtime-файлы `xray.exe`, `geoip.dat`, `geosite.dat`, `configs/*.json`, если пользователь явно не просит.
...
- лимит zip - `120 MB`;
- из архива извлекаются только `xray.exe`, `geoip.dat`, `geosite.dat`;
...
Оставшийся риск: checksum/signature скачанного asset сейчас не проверяется. Если это потребуется, добавляйте проверку по официальному источнику Xray-core и не ослабляйте allowlist файлов.
