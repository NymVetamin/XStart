...
Кнопка `Import` открывает диалог, который принимает либо `vless://` ссылку, либо полный Xray JSON. JSON должен содержать VLESS outbound с `settings.vnext[0].address` и `port`; импорт нормализует tag VLESS outbound в `vless-reality`, добавляет локальный `socks-in`, если его нет, и добавляет route `socks-in -> vless-reality`, если его нет. Имена профилей и файлов уникализируются, существующие конфиги не перезаписываются.
