...
Маршрутизация вставляет сначала правило `tun-in` с link-local/private/multicast/broadcast/loopback ranges -> `block`, затем общее правило `tun-in -> vless-reality`, чтобы internet TUN-трафик не перехватывался более ранними direct/block правилами базового профиля и локальный Windows-шум не отправлялся на proxy. DNS назначается на сам TUN-адаптер через `netsh`, поэтому DNS-пакеты тоже входят через `tun-in` и идут по тому же outbound, без отдельного direct DNS outbound.
