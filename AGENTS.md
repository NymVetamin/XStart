...
- title: `XStart`;
- стартовый размер: `1080x760`;
- минимальный размер: `960x680`;
- тема ttk: `clam`;
- интерфейс полностью на русском.

Основные зоны:

- слева `Listbox` профилей и кнопки `Импорт` / `Удалить`;
- справа рабочая область: `Управление`, широкая панель `Подключение`, ниже `Лог Xray`;
- в управлении кнопка `Запустить Xray` / `Остановить Xray`, статус, чекбокс `TUN-режим`, кнопка `Загрузить ядро`;
- информация о прокси выводится широкой таблицей, чтобы длинные server/SNI/fingerprint не вылезали из панели.
...
5. Выбрать профиль и нажать `Запустить Xray`.
6. Проверить статус, логи и порт `127.0.0.1:10808`.
7. Нажать `Остановить Xray` и убедиться, что процесс завершен.
...
Маршрутизация вставляет сначала правило `tun-in` с link-local/private/multicast/broadcast/loopback ranges -> `tun-block`, затем общее правило `tun-in -> vless-reality`, чтобы internet TUN-трафик не перехватывался более ранними direct/block правилами базового профиля и локальный Windows-шум не отправлялся на proxy. Используйте отдельный `tun-block`, а не общий `block`, чтобы не конфликтовать с импортированными JSON-конфигами. DNS назначается на сам TUN-адаптер через `netsh`, поэтому DNS-пакеты тоже входят через `tun-in` и идут по тому же outbound, без отдельного direct DNS outbound.

В Xray `v26.3.27` Windows TUN создает Wintun-адаптер, но не применяет `gateway`, `dns`, `autoSystemRoutingTable` и `autoOutboundsInterface`: эти поля есть в свежем `main`, но отсутствуют в исходнике тега `v26.3.27`. Поэтому приложение делает совместимый fallback: до запуска добавляет точный host-route до proxy endpoint через физический маршрут, после появления `xstart0` назначает IPv4/DNS/interface metric через `netsh` и добавляет `0.0.0.0/0` + `::/0` на `xstart0` через `New-NetRoute`. `New-NetIPAddress`/`Set-DnsClientServerAddress`/`Set-NetIPInterface` на Wintun в этом окружении нестабильны, поэтому не используйте их без повторной live-проверки. Все маршруты создаются в `ActiveStore`, сохраняются в `current_tun_routes` и удаляются через `cleanup_tun_routes()` при ошибке, остановке или неожиданном завершении Xray. Не возвращайте старый route-worker и не добавляйте split-default без проверки живого route table.
...
Кнопка `Импорт` открывает диалог, который принимает либо `vless://` ссылку, либо полный Xray JSON. JSON должен содержать VLESS outbound с `settings.vnext[0].address` и `port`; импорт нормализует tag VLESS outbound в `vless-reality`, добавляет локальный `socks-in`, если его нет, и добавляет route `socks-in -> vless-reality`, если его нет. Имена профилей и файлов уникализируются, существующие конфиги не перезаписываются.
